...
#import "VTBEncDecViewController.h"
...
@property (nonatomic,strong)XXPLEAGLLayer *playLayer;  //解码后播放layer
...
//编解码器
@property (nonatomic,strong)XXH264DecodeTool *h264Decoder;
@property (nonatomic,strong)XXH264EncodeTool *h264Encoder;

@end

@implementation VTBEncDecViewController
...
- (void)configH264Encoder{
    if (!self.h264Encoder) {
        self.h264Encoder = [[XXH264EncodeTool alloc]init];
        //640 * 480
...
- (void)configH264Decoder{
    if (!self.h264Decoder) {
        self.h264Decoder = [[XXH264DecodeTool alloc] init];
...
- (void)initPlayLayer{
    CGFloat height = (self.view.frame.size.height - 100)/2.0 - 20;
    CGFloat width = self.view.frame.size.width - 100;
    self.playLayer = [[XXPLEAGLLayer alloc] initWithFrame:CGRectMake(100, (self.view.frame.size.height - 100)/2.0 + 100,width,height)];
...
- (UIButton *)startBtn{
    if (!_startBtn) {
        _startBtn = [[UIButton alloc]initWithFrame:CGRectMake(220, 80, 100, 50)];
...
- (UILabel *)titleLabel{
    if (!_titleLabel) {
        _titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(50, 90, 150, 30)];
