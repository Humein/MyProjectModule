...
/*
这种编码之后，视频比之前会压缩多少倍？
 - 不同视频源压缩比例不一样。一般20倍是可以的。

*/

#import "VTBEncodeViewController.h"
#import <AVFoundation/AVFoundation.h>
#import <VideoToolbox/VideoToolbox.h>

static NSString *const H264FilePath = @"sandbox.h264";

@interface VTBEncodeViewController ()<AVCaptureVideoDataOutputSampleBufferDelegate>
{
    int frameNO;//帧号
    //录制队列
...
@implementation VTBEncodeViewController
...
void vtbdidCompressH264(void *outputCallbackRefCon, void *sourceFrameRefCon, OSStatus status, VTEncodeInfoFlags infoFlags, CMSampleBufferRef sampleBuffer) {
    ...
    VTBEncodeViewController* encoder = (__bridge VTBEncodeViewController*)outputCallbackRefCon;
...
- (void)configFileHandle{
    //从摄像头获取到视频数据，然后再编码成H264裸数据保存在沙盒中。
...
- (UILabel *)titleLabel{
    if (!_titleLabel) {
        _titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(50, 80, 150, 30)];
